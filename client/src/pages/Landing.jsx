...
const Landing = ({ showAbout, setShowAbout }) => {
  ...
  return (
    <div className="landing-container" style={{ position: 'relative', minHeight: '100vh', overflow: 'hidden', fontSize: '1.13rem' }}>
      {/* If user is logged in, show a redirect message */}
      {user && user.email ? (
        <div style={{
          position: 'relative',
          zIndex: 2,
          ...
          <div style={{
            ...
            <h1 style={{ color: '#0a2342', marginBottom: '1rem', fontSize: '2.5rem' }}>Welcome back, {user.name || 'User'}!</h1>
            <p style={{ color: '#334155', fontSize: '1.2rem', marginBottom: '2rem' }}>
              You're already logged in. Redirecting you to your personalized dashboard...
            </p>
            <button 
              onClick={() => navigate('/')}
              style={{
